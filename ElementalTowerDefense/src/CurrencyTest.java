...
public class CurrencyTest {
	...
	public void testIncrementAndDecrementCurrency() {
		...
		// If a user tries to decrement Currency past what a player has,
		// decCurrency
		// should return false.
	...
	public void testCurrencyIncrementsWhenEnemyDies() {
		...
		Enemy enemy_fire;
		enemy_fire = new EnemyFire(new Point2D.Double(0, 0), 100, 100);
		...
		// enemy_fire.kill(player); // Kills the enemy. Only one element type
		// needs to be tested b/c kill is a static method.

		map.killEnemy(enemy_fire, player);

		assertEquals(this.player.getCurrency(),
				(oldCurrency + enemy_fire.getWorth()));
	...
	public void testCurrencyDecrementWhenPlayerUpgradesATower() {
		...
		assertTrue(tower_fire.upgrade(player)); // Return true if the tower can
												// be upgraded
		assertEquals(this.player.getCurrency(), (oldCurrency - tower_fire.cost));

		oldCurrency = tower_fire.cost - 1;
		this.player.setCurrency(oldCurrency);
		assertFalse(tower_fire.upgrade(player)); // Return false if the tower
													// can't be upgraded
		assertEquals(this.player.getCurrency(), oldCurrency); // Make sure the
																// player's
																// currency
																// didn't
																// change.
